...
let access board x y =
  ...
  match (x, y) with
  | x, y when (0 <= x && x < xn) && 0 <= y && y < yn -> Some (x, y)
  | _ -> None
...
let span board x y dirs =
  let rec span_dir x y acc (dx, dy) =
    ...
    | Filled _ -> acc
    | Empty | Light | Shined ->
        span_dir (x + dx) (y + dy) ((x, y) :: acc) (dx, dy)
  in
  dirs
  |> List.fold_left
       (fun acc (dx, dy) -> span_dir (x + dx) (y + dy) acc (dx, dy))
       [ (x, y) ]
...
let solve board =
  ...
  let name x y =
    access board x y |> Option.map (fun (x, y) -> names.(x).(y)) |> Option.join
  in
  let neighbors x y =
    [ (0, 1); (0, -1); (-1, 0); (1, 0) ]
  ...
  let fassume x = x |> F.make_cnf |> assume in
  let assert_n n terms =
    (* assert n lights *)
    let l = List.length terms in
    let negate_excpt_n terms i =
      terms
      |> List.mapi (fun j t ->
             if (0 <= j - i && j - i < n) || (0 <= j + l - i && j + l - i < n)
             then t
             else F.make_not t)
    in
    match n with
    | 0 ->
        begin
          terms |> List.map F.make_not |> F.make_and
        end
         ...
         Array.iteri (fun y -> function
           | Filled i ->
               begin
                 ...
                 |> List.map F.make_atom |> assert_n i |> fassume
               end
           | Light | Shined | Empty -> ()));
           ...
           Array.iteri (fun y _ ->
               ...
               | Filled _ -> begin
                   ...
                   try
                     ...
                     |> List.map (fun (x, y) -> name x y)
                     |> List.map Option.get (* should all be non-none *)
                     |> List.map F.make_atom |> assert_max_one |> fassume
                   with _ -> ()
                 end
               | Light | Shined | Empty -> ()))
  in
  ...
  | Sat state ->
      Some
        ...
        |> Array.mapi (fun x ->
               Array.mapi (fun y -> function
                 | Light | Shined | Empty ->
                     if name x y |> Option.get |> state.Msat.eval then Light
                     else Shined
                 | Filled i -> Filled i)))
